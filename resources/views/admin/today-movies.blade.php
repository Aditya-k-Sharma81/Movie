...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full flex-grow">
    ...
    <div class="mb-10 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
            ...
            <h2 class="text-3xl font-black text-white">
                {{ \Carbon\Carbon::parse($today)->isToday() ? 'Movies playing today' : 'Movies on ' . \Carbon\Carbon::parse($today)->format('d M Y') }}
            </h2>
        </div>
        
        <form method="GET" action="{{ route('admin.today.movies') }}" class="flex items-center gap-3">
            <div class="relative">
                <input type="date" name="date" value="{{ $today }}" onchange="this.form.submit()" 
                       class="bg-slate-900 border border-slate-700 text-slate-300 text-sm rounded-xl focus:ring-amber-500 focus:border-amber-500 block w-full pl-3 pr-4 py-2 cursor-pointer outline-none transition-all hover:border-amber-500/50">
            </div>
            @if(!\Carbon\Carbon::parse($today)->isToday())
                <a href="{{ route('admin.today.movies') }}" class="px-4 py-2 text-sm bg-slate-800 hover:bg-slate-700 text-white rounded-xl transition-all border border-slate-700">
                    Reset
                </a>
            @endif
        </form>
    ...
    @if($todayMovies->isEmpty())
        <div class="glass border border-dashed border-slate-800 rounded-3xl p-20 text-center">
            ...
            <p class="text-slate-500 text-sm">There are no movies scheduled to play {{ \Carbon\Carbon::parse($today)->isToday() ? 'today' : 'on ' . \Carbon\Carbon::parse($today)->format('d M Y') }}.</p>
