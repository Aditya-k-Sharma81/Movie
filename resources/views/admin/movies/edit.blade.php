        ...
        <div class="glass rounded-3xl p-5 sm:p-8">
            <form id="editMovieForm" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ $movie->title }}" class="form-input w-full px-4 py-3 rounded-xl" placeholder="e.g. Inception">
                </div>
                ...
                <div>
                    ...
                    <select name="category[]" multiple class="select2-multiple w-full">
                        <option value="Bollywood" {{ in_array('Bollywood', $movie->category) ? 'selected' : '' }}>Bollywood</option>
                        <option value="Hollywood" {{ in_array('Hollywood', $movie->category) ? 'selected' : '' }}>Hollywood</option>
                        <option value="Tollywood" {{ in_array('Tollywood', $movie->category) ? 'selected' : '' }}>Tollywood</option>
                        <option value="Anime" {{ in_array('Anime', $movie->category) ? 'selected' : '' }}>Anime</option>
                        <option value="Regional" {{ in_array('Regional', $movie->category) ? 'selected' : '' }}>Regional</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="genre[]" multiple class="select2-multiple w-full">
                        <option value="Action" {{ in_array('Action', $movie->genre) ? 'selected' : '' }}>Action</option>
                        <option value="Comedy" {{ in_array('Comedy', $movie->genre) ? 'selected' : '' }}>Comedy</option>
                        <option value="Drama" {{ in_array('Drama', $movie->genre) ? 'selected' : '' }}>Drama</option>
                        <option value="Sci-Fi" {{ in_array('Sci-Fi', $movie->genre) ? 'selected' : '' }}>Sci-Fi</option>
                        <option value="Horror" {{ in_array('Horror', $movie->genre) ? 'selected' : '' }}>Horror</option>
                        <option value="Romance" {{ in_array('Romance', $movie->genre) ? 'selected' : '' }}>Romance</option>
                        <option value="Thriller" {{ in_array('Thriller', $movie->genre) ? 'selected' : '' }}>Thriller</option>
                    </select>
                </div>
                ...
                <div class="sm:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 p-4 rounded-xl border border-slate-800 bg-slate-900/30">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="start_time" value="{{ \Carbon\Carbon::parse($movie->start_time)->format('Y-m-d\TH:i') }}" class="form-input w-full px-4 py-3 rounded-xl">
                    </div>

                    <!-- End Date & Time -->
                    <div>
                        <label class="block text-xs font-medium text-slate-500 uppercase mb-2">End Date & Time</label>
                        <input type="datetime-local" name="end_time" value="{{ \Carbon\Carbon::parse($movie->end_time)->format('Y-m-d\TH:i') }}" class="form-input w-full px-4 py-3 rounded-xl">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="release_date" value="{{ \Carbon\Carbon::parse($movie->release_date)->format('Y-m-d') }}" class="form-input w-full px-4 py-3 rounded-xl">
                </div>

                <!-- Language -->
                <div>
                    <label class="block text-xs font-medium text-slate-500 uppercase mb-2">Language</label>
                    <input type="text" name="language" value="{{ $movie->language }}" class="form-input w-full px-4 py-3 rounded-xl" placeholder="Hindi, English">
                </div>
        ...
        $(document).ready(function() {
            ...
            $('#editMovieForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        let errorMessage = xhr.responseJSON?.message || 'An error occurred while updating the movie.';
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
                            text: errorMessage,
