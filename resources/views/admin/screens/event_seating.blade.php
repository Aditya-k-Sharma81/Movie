...
@section('title', 'Event Seating Arrangement Builder')
@section('page_title', 'Event Seating Arrangement Builder')
...
<div class="flex-1 p-4 md:p-8 flex flex-col xl:flex-row gap-8">
                ...
                <div class="w-full xl:w-80 flex-shrink-0 space-y-6">
                    ...
                    <div class="glass p-6 rounded-2xl border border-slate-800">
                        <button onclick="saveLayout()" class="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-emerald-500/20 flex items-center justify-center">
                            <i class="fa-solid fa-save mr-2"></i> Save Event Seating Layout
                        </button>
                    </div>
                </div>
                ...
                <div class="flex-1 glass p-8 rounded-2xl border border-slate-800 flex flex-col items-center justify-center overflow-x-auto">
                    ...
                    <div class="w-3/4 max-w-2xl h-12 mb-16 relative flex items-center justify-center">
                        ...
                        <span class="relative text-white/50 text-sm font-bold tracking-widest uppercase">Stage/Screen</span>
                    </div>
...
@section('scripts')
    <script>
        ...
        let layout = @json($admin->event_seating_layout['layout'] ?? []);
        ...
        function initLayout() {
            const savedData = @json($admin->event_seating_layout ?? null);
            if (savedData) {
        ...
        function saveLayout() {
            ...
            $.ajax({
                url: "{{ route('admin.screens.event_seating.save') }}",
                type: "POST",
                ...
                success: function(response) {
                    if (response.status) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Event Layout Saved!',
                            text: 'Event seating layout has been saved successfully.',
