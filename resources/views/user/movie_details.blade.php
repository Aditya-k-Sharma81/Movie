...
<div id="bookingModal" class="fixed inset-0 z-[100] hidden flex flex-col bg-slate-950 overflow-hidden">
    ...
    <div class="flex items-center justify-between px-6 py-4 bg-slate-900/50 border-b border-slate-800 backdrop-blur-xl shrink-0">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-white font-black text-lg leading-none mb-1">{{ $movie->title }}</h2>
                <p class="text-[10px] text-slate-500 uppercase font-black tracking-widest">{{ \Carbon\Carbon::parse($movie->start_time)->format('d M | h:i A') }}</p>
            </div>
        </div>
    ...
    <div class="flex flex-grow overflow-hidden">
        ...
        <div class="flex-grow overflow-auto p-8 flex flex-col items-center bg-slate-950/40">
            ...
            <div class="w-full max-w-3xl flex flex-col items-center mb-14">
                ...
                <p class="text-[10px] text-slate-600 uppercase font-black tracking-[0.5em]">Screen This Way</p>
            ...
            <div class="mt-16 flex flex-wrap justify-center gap-6 px-6 py-3 bg-slate-900/30 rounded-2xl border border-slate-800">
                ...
                <div class="flex items-center gap-2"><div class="w-3.5 h-3.5 rounded bg-indigo-600"></div><span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Standard ₹{{ $movie->price_normal }}</span></div>
                <div class="flex items-center gap-2"><div class="w-3.5 h-3.5 rounded bg-pink-500"></div><span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Premium ₹{{ $movie->price_premium }}</span></div>
                <div class="flex items-center gap-2"><div class="w-3.5 h-3.5 rounded bg-yellow-500"></div><span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">VIP ₹{{ $movie->price_vip }}</span></div>
            </div>
        </div>
...
<script>
    ...
    const PRICES = {
        standard: {{ $movie->price_normal }},
        premium: {{ $movie->price_premium }},
        vip: {{ $movie->price_vip }}
    };
    ...
    function fetchSeats() {
        $.get(`/movie/${MOVIE_ID}/seats`, function(data) {
            if(data.status) {
    ...
    function toggleSeat(seat) {
        ...
        updateFooter();
        // Force re-render to apply selected class
