@foreach($events as $event)
    <a href="{{ route('event.details', ['id' => $event->_id]) }}" class="group cursor-pointer event-card block"
        data-start-time="{{ \Carbon\Carbon::parse($event->start_time)->toIso8601String() }}">
        <div
            ...
            <img src="{{ $event->poster ?? 'https://via.placeholder.com/400x600?text=No+Poster' }}"
